...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name("q")).sendKeys("honda s2000", Key.RETURN);
  ...
  await driver
    ...
    .sendKeys("honda civic type-r", Key.RETURN);

  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("honda civic type-r - Google Search"), 1000);
});
