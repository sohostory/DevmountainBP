...
const addBackgroundColor = () => {
  sayHelloButton.classList.add("black-background");
};

sayHelloButton.addEventListener("mouseover", addBackgroundColor);
...
const removeBackgroundColor = () => {
  sayHelloButton.classList.remove("black-background");
};

sayHelloButton.addEventListener("mouseout", removeBackgroundColor);
...
/*
    ...
    Outside of the function, select the button with the id "repeat-button" and add a click event 
...
const queryTest = () => {
  let testQuery = "";
  axios.get(`http://localhost:3000/query-test/${testQuery}`).then((res) => {
    console.log(res.data);
  });
};
...
/* 
    ...
    On each iteration of the loop, create a new p element. Set its textContent equal the string 
    at the current index (i) and then append the new p element onto the document's body. 
*/
...
/*
    ...
    shows error with you sent an empty query

    2: Send more than 1 query on the URL -- what happened? 
    still reads as 1 query
*/
...
/*
    ...
    Now make an axios post request to /food. Inside the parentheses where you passed the URL in, 
